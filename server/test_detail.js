...
async function test() {
  ...
  console.log("1. 测试歌手详情API...");
  const detailUrl = `https://music.163.com/api/artist/detail?id=${artistId}`;
  ...
  try {
    ...
    if (res.data.code === 200 && res.data.data?.artist) {
      const artist = res.data.data.artist;
      console.log("\n歌手名:", artist.name);
      console.log("accountId:", artist.accountId);
      console.log("id:", artist.id);
    } else {
      console.log("\nAPI返回:", JSON.stringify(res.data, null, 2).substring(0, 500));
    }
  } catch (e) {
