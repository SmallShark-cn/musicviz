...
async function testFansAPI() {
  ...
  for (const artist of artists) {
    console.log(`\n=== 测试 ${artist.name} (${artist.id}) ===");
    
    // 尝试 POST 请求
    try {
      const url = "https://music.163.com/api/artist/follow/count";
      const res = await axios.post(url, `id=${artist.id}`, {
        headers: {
          "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120.0.0.0 Safari/537.36",
          "Content-Type": "application/x-www-form-urlencoded",
          "Cookie": cookie,
          "Referer": `https://music.163.com/artist?id=${artist.id}`,
          "csrf_token": "5926969fd5ec7a23456bb83865e837ff",
        },
        timeout: 10000,
      });
    ...
    // 尝试 GET 请求
    try {
      const url = `https://music.163.com/api/artist/follow/count?id=${artist.id}`;
...
testFansAPI().catch(console.error);
